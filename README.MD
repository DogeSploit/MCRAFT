...
Zip files and folders are supported. Just drag and drop them into the browser window. You can open folders in read-only and read-write mode. New chunks may be generated incorrectly for now.
...
- `?lockConnect=true` - Only works when the `ip` parameter is set. Disables cancel/save buttons and all inputs in the connect screen already set as parameters. Useful for integrates iframes.
